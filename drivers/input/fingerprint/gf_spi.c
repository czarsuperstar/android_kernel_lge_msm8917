...
#if 1 // LGE_CHANGES - for pin check test
...
static int gf_check_irq_rst(struct gf_dev *gf_dev)
{
	...
	for(i = 0;i < MAX_RETRY_HW_CHECK_COUNT;i++)
    {
		...
		gf_dbg("rst pin check success.\n");
    	gf_spi_read_word(gf_dev,GF_IRQ_CTRL3,&reset_irq_data);

    	gf_dbg("%s: reset_irq_data = 0x%x \n",__func__,reset_irq_data);
		if(reset_irq_data != GF_3208_RST_DATA)
		{
			gf_dbg("irq pin check fail.\n");
			continue;
		}else{
			gf_dbg("irq pin check success.\n");
			gf_dev->device_available = 1;
			return 0;
		}
    }
...
#if 1 // LGE_CHANGES - support key event
		//input_report_key(gf_dev->input, gf_key.key, gf_key.value);
		gf_dbg("KEY_HOME =%d, gf_key.value = %d", KEY_PROGRAM, gf_key.value);
...
static void gf_reg_key_kernel(struct gf_dev *gf_dev)
{
	...
	set_bit(KEY_PROGRAM, gf_dev->input->keybit); // LGE_CHANGES - support key event
...
static int gf_regulator_init(struct gf_dev *data)
{
    int error = 0;
    struct regulator *vreg;

    data->vreg = NULL;
    vreg = regulator_get(&data->spi->dev, "goodix,vddio");
    if (IS_ERR(vreg)) {
        error = PTR_ERR(vreg);
...
static int gf_regulator_set(struct gf_dev *data, bool enable)
{
    int error = 0;
    ...
    if(enable) {
        if(!data->power_on){
            error = regulator_enable(data->vreg);
			dev_info(&data->spi->dev, "[INFO] regulator_enable on \n");
        }
    } else {
        if(data->power_on){
            error = regulator_disable(data->vreg);			
			dev_info(&data->spi->dev, "[INFO] regulator_enable off \n");
        }
    }

    if(error < 0)
		dev_err(&data->spi->dev, "can't set(%d) regulator, error(%d)", enable, error);
    else
        data->power_on = enable;


	dev_info(&data->spi->dev, "[INFO] regulator value : %d", regulator_get_voltage(data->vreg));
    return error;
}
...
#endif
{
...
#if 1 // Chargerlogo sleep current improvement
	...
	ret = gf_check_irq_rst(gf_dev);
	if(ret)
	{
		gf_dbg("%s gf_check_irq_rst failed,probe failed!\n",__func__);
		//return -1;
	}
	gfspi_ioctl_clk_disable(gf_dev);
    mdelay(1);
    ...
    mdelay(1);
			...
			if(!gf_dev->lge_input) {
				dev_err(&gf_dev->spi->dev, "[ERROR] input_allocate_deivce failed.\n");
				status = -ENOMEM;
				//goto error;
			}
			...
			if(status) {
				dev_err(&gf_dev->spi->dev, "[ERROR] nput_register_device failed.\n");
				input_free_device(gf_dev->lge_input);
				//goto error;
			}

			if(sysfs_create_group(&gf_dev->lge_input->dev.kobj, &gf_attr_group)) {
				dev_err(&gf_dev->spi->dev, "[ERROR] sysfs_create_group failed.\n");
				//goto err_sysfs;
			}
