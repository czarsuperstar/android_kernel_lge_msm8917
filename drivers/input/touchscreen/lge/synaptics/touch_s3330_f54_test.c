...
int CompareAbsRawReport(u16 input)
{
	...
	for (i = 0; i < RxChannelCount + F12_2DTxCount; i++) {
		...
		if (input == eRT_AbsLpwgRaw && AbsSigned32Data[i] == 0)
			continue;
...
int CompareHybridAbsRawReport(void)
{
	bool result = true;
	int i, k = 0;
...
int CompareLpwgJitterReport(void)
{
	bool result = true;
	int i = 0;
	int max_count = 0;
	...
	for (i = 1; i < 26; i++) {
		if (LpwgJitterMax[i - 1] >= LpwgJitterMaxLimit) {
			...
			if ((temp_fail == 1) && (fail_count > 5)) {
				max_count = fail_count;
			}
			temp = i;
		} else {
			fail_count = 0;
...
int ReadAbsRawReport(struct device *dev, u16 input)
{
	...
	result = CompareAbsRawReport(input);
	write_file(dev, f54buf, TIME_INFO_SKIP);
...
int JitterTestPreparation(struct device *dev)
{
	int ret = 0;
	unsigned char data;
	unsigned int count = 0;
...
/* Rawdata Jitter */
int JitterTest(struct device *dev, int mode, char *buf)
{
	int ret = 0;
	int i, j, k, z = 0;
...
int LpwgJitterTest(struct device *dev)
{
	...
	for (i = 0; i < 25; i++) {
		Read8BitRegisters(dev, (F54DataBase + 16), &buf[0], sizeof(buf));
	...
	result = CompareLpwgJitterReport();
