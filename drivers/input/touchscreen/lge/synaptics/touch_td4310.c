...
void td4310_reset_ctrl(struct device *dev, int ctrl)
{
	...
	case SW_RESET:
		TOUCH_I("%s : SW Reset\n", __func__);
		td4310_write(dev, DEVICE_COMMAND_REG, &wdata, sizeof(u8));
...
static int td4310_abs_enable(struct device *dev, bool enable)
{
	struct td4310_data *d = to_td4310_data(dev);
	...
	ret = td4310_read(dev, INTERRUPT_ENABLE_REG, &val, sizeof(val));
	if (ret < 0) {
	...
	ret = td4310_write(dev, INTERRUPT_ENABLE_REG, &val, sizeof(val));
	if (ret < 0) {
...
static int td4310_get_f12(struct device *dev)
{
	struct td4310_data *d = to_td4310_data(dev);
...
static int td4310_page_description(struct device *dev)
{
	...
	unsigned short pdt;
	int ret;
	/* td4310 fw recovery - slave addr change to 0x2c */
	...
	memset(&d->f11, 0, sizeof(struct td4310_function));
	memset(&d->f12, 0, sizeof(struct td4310_function));
	memset(&d->f1a, 0, sizeof(struct td4310_function));
...
static void td4310_sensing_cmd(struct device *dev, int type)
{
	TOUCH_TRACE();
	...
	switch (type) {
	case ABS_STOP:
		td4310_abs_enable(dev, false);
		break;
	case ABS_SENSING:
		td4310_abs_enable(dev, true);
		break;
	case WAKEUP_STOP:
...
static int td4310_tci_password(struct device *dev)
{
	TOUCH_TRACE();
	return td4310_tci_knock(dev);
}

static int td4310_tci_active_area(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4310_data *d = to_td4310_data(dev);
	u8 buffer[8];
	...
	td4310_write(dev, d->f12_reg.ctrl[18], buffer, sizeof(buffer));

	return 0;
}

static void td4310_tci_area_set(struct device *dev, int cover_status)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int td4310_lpwg_mode(struct device *dev)
{
	...
	if (atomic_read(&ts->state.fb) == FB_SUSPEND) {
		if (ts->role.mfts_lpwg) {
			...
			td4310_tci_area_set(dev, QUICKCOVER_OPEN);
			td4310_lpwg_control(dev, ts->lpwg.mode);
			td4310_sensing_cmd(dev, WAKEUP_SENSING);
			...
			if (atomic_read(&ts->state.sleep) == IC_DEEP_SLEEP)
				td4310_sleep_control(dev, IC_NORMAL);
			td4310_sensing_cmd(dev, WAKEUP_STOP);
			td4310_lpwg_debug(dev, TCI_1);
			...
			if (atomic_read(&ts->state.sleep) == IC_DEEP_SLEEP)
				td4310_sleep_control(dev, IC_NORMAL);
			td4310_tci_area_set(dev, QUICKCOVER_CLOSE);
	...
	if (ts->lpwg.screen) {
		...
		TOUCH_I("%s(%d) - normal\n",
				__func__, __LINE__);
		td4310_lpwg_control(dev, LPWG_NONE);
		td4310_sensing_cmd(dev, ABS_SENSING);
	} else if (ts->lpwg.mode == LPWG_NONE) {
		/* normal */
		TOUCH_I("%s(%d) - normal on screen off\n",
				__func__, __LINE__);
		td4310_lpwg_control(dev, LPWG_NONE);
		td4310_sensing_cmd(dev, ABS_STOP);
	} else if (ts->lpwg.sensor == PROX_NEAR) {
		/* wake up */
		...
		if (ts->lpwg.qcover == HOLE_NEAR)
			...
			td4310_tci_area_set(dev, QUICKCOVER_OPEN);
		td4310_lpwg_control(dev, LPWG_DOUBLE_TAP);
		td4310_sensing_cmd(dev, PARTIAL_WAKEUP_SENSING);
	}
...
static int td4310_get_status(struct device *dev, u8 *interrupt)
{
	struct td4310_data *d = to_td4310_data(dev);
	u8 status[2] = {0, };
	int ret;
	...
	ret = td4310_read(dev, DEVICE_STATUS_REG,
...
static int td4310_irq_clear(struct device *dev)
{
	...
	return td4310_get_status(dev, NULL);
}
...
static int td4310_get_object_count(struct device *dev)
{
	struct td4310_data *d = to_td4310_data(dev);
	...
	ret = td4310_read(dev, d->f12_reg.data[15],
			(u8 *) buf, sizeof(buf));
...
static int td4310_irq_abs_data(struct device *dev, int object_to_read)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4310_data *d = to_td4310_data(dev);
	struct td4310_object objects[MAX_NUM_OF_FINGERS];
	struct td4310_object *obj;
...
static int td4310_irq_abs(struct device *dev)
{
	...
	object_to_read = td4310_get_object_count(dev);
	...
	return td4310_irq_abs_data(dev, object_to_read);
}
...
static int td4310_irq_handler(struct device *dev)
{
	...
	ret = td4310_get_status(dev, &irq_status);
...
static int td4310_get_cmd_version(struct device *dev, char *buf)
{
	...
	offset += snprintf(buf + offset, PAGE_SIZE - offset,
			"IC_product_id[%s]\n", d->ic_info.product_id);
	if (td4310_is_product(d, "PLG632", 6) || td4310_is_product(d, "PLG635", 6))
		offset += snprintf(buf + offset, PAGE_SIZE - offset,
			"Touch IC : TD4310\n\n");
	else
		offset += snprintf(buf + offset, PAGE_SIZE - offset,
	...
	if (td4310_is_img_product(d, "PLG632", 6) || td4310_is_img_product(d, "PLG635", 6))
		offset += snprintf(buf + offset, PAGE_SIZE - offset,
			"Touch IC : TD4310\n\n");
	else
		offset += snprintf(buf + offset, PAGE_SIZE - offset,
...
static int td4310_fw_compare(struct device *dev, const struct firmware *fw)
{
	...
	if (ts->force_fwup) {
		update = 1;
	} else if (binary->major && device->major) { /* Only check test version */
		if (binary->minor != device->minor)
			update = 1;
		else if (binary->build > device->build)
			update = 1;
	} else if (binary->major ^ device->major) { /* Check compare Test/Official version */
		update = 1;
	} else if (!binary->major && !device->major) { /* Only check official version */
		if (binary->minor != device->minor)
			update = 1;
	}
