...
/* Start - kiwoo.han 2016-9-19 */
static int TD4100_Get_AMP_Short_Data(struct device *dev)
{
	struct synaptics_data *d = to_synaptics_data(dev);
	int i, j, k, ret = 0;
	int result = 0;
	...
	ret = TD4100_Get_Report_Data(dev, REPORT_TYPE_RAW_DATA, shortdata, REPORT_DATA_LEN);
	if (ret < 0) {
		TOUCH_E("Error to get report data\n");
		goto FAIL;
	}
	...
	ret = synaptics_read(dev, TDDI_CBC_ENABLE_CTRL_REG, get_left_cbc, sizeof(get_left_cbc)); /* F54_ANALOG_CTRL103 (TDDI CBC Enable) */
	ret |= synaptics_read(dev, TDDI_CBC_ENABLE_CTRL_REG, get_right_cbc, sizeof(get_right_cbc)); /* F54_ANALOG_CTRL103 (TDDI CBC Enable) */
	if (ret < 0) {
	...
	ret |= synaptics_write(dev, TDDI_CBC_ENABLE_CTRL_REG, get_left_cbc, sizeof(get_left_cbc)); /* F54_ANALOG_CTRL103 (TDDI CBC Enable) */
	if (ret < 0) {
	...
	ret |= synaptics_write(dev, TDDI_CBC_ENABLE_CTRL_REG, get_right_cbc, sizeof(get_right_cbc)); /* F54_ANALOG_CTRL103 (TDDI CBC Enable) */
	if (ret < 0) {
...
static int TD4100_Test_Result_Data(struct device *dev, int test_type, int* test_result)
{
	...
	*test_result = TEST_FAIL;
	...
	case P2P_NOISE_TEST:
		...
		break;
	case AMP_SHORT_TEST:
		result = TD4100_Get_AMP_Short_Data(dev);
		if(result < 0) {
			TOUCH_E("TD4100_Get_AMP_Short_Data error\n");
			return TEST_FAIL;
		}
...
static ssize_t show_amp_short(struct device *dev, char *buf)
{
	...
	ret_size = TD4100_Test_Result_Data(dev, AMP_SHORT_TEST, &test_result);
	memcpy(buf, buffer, ret_size);
...
static int synaptics_sd(struct device *dev, char *buf, int *raw, int *ch)
{
	...
	if(!strcmp(panel, "Tovis")) {
		...
		/* Start - kiwoo.han 2016-10-1 */
		#if 0 // TBD
		ret += TD4100_Test_Result_Data(dev, AMP_SHORT_TEST, &short_result);
		touch_msleep(30);
		write_file(dev, buffer, TIME_INFO_SKIP);
		#endif
		/* End - kiwoo.han 2016-10-1 */
