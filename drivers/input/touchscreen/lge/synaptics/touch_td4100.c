...
/* model definition will be deleted after define sf3 value */
...
static int synaptics_irq_status(struct device *dev, u8 dev_status)
{
	...
	if (dev_status == 0x89 || dev_status == 0x09) {
		TOUCH_I("###### ESD Detected !!! - Call ESD Recovery Func ######\n");
		return -ERESTART;
	}
...
static int synaptics_irq_handler(struct device *dev)
{
	...
	if (ret < 0) {
		TOUCH_E("synaptics get status error : %d\n", ret);
		return ret;
	}
