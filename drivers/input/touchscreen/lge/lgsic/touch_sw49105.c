...
static int sw49105_fb_notifier_callback(struct notifier_block *self,
...
static int sw49105_tci_control(struct device *dev, int type)
{
	...
	case ACTIVE_AREA_CTRL:
		ret = sw49105_tci_active_area(dev, 0, 0, 720, 1280); /* 720 x 1280 */
		/*
				ts->tci.area.x1,
				ts->tci.area.y1,
				ts->tci.area.x2,
...
int sw49105_tc_driving(struct device *dev, int mode)
{
	...
	TOUCH_I("sw49105_tc_driving = %d, 0x%x\n", mode,ctrl);
	sw49105_reg_read(dev, spr_subdisp_st, (u8 *)&rdata, sizeof(u32));
			     ...
			     const struct firmware *fw)
{
	...
	TOUCH_I("%s - START\n", __func__);
	TOUCH_I("FW Image Size : 0x%8.8X(%d)\n",
...
static int sw49105_init(struct device *dev)
{
	...
	if (atomic_read(&ts->state.core) == CORE_PROBE) {
		...
		ts->fb_notif.notifier_call = sw49105_fb_notifier_callback;
...
int sw49105_check_status(struct device *dev)
{
	...
	TOUCH_I("Normal device status info check(0x%x)\n",
		(status & MASK_NORMAL_DEVICE_STATUS));

	if (!(status & (1 << 5))) {
		TOUCH_I("Abnormal device status detected(%x)\n", status);
		ret = -ERESTART;
	} else if(status & MASK_ABNORMAL_DEVICE_STATUS) {
		TOUCH_I("Abnormal device status detected(0x%x)\n", status);
		ret = -ERESTART;
	}else if (status & INT_LOGGING_CLR_BIT) {
