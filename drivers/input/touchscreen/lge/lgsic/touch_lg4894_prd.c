	...
	if (type == SHORT_NODE_TEST) {
		for (i = 0; i < ROW_SIZE; i++) { //ROW_SIZE = 26
			for (j = 0; j < col_size; j++) { //col_size = 2 //M1_COL_SIZE(2)+1; --> j < 2
				...
				TOUCH_I("%d / %d / %d \n",LowerImage[i][j], short_buf[i*col_size + j], UpperImage[i][j]); //test
		...
		for (i = 0; i < ROW_SIZE; i++) {
			for (j = 0; j < col_size-1; j++) {
				TOUCH_I("%d / %d / %d \n",LowerImage[i][j],open_buf[i*col_size+j],UpperImage[i][j]); //test
...
static int prd_compare_rawdata(struct device *dev, u8 type)
{
	...
	if (type == U0_M1_RAWDATA_TEST) {
		for (i = 0; i < ROW_SIZE; i++) {
			for (j = 0; j < col_size-1; j++) {
				TOUCH_I("%d / %d / %d\n",LowerImage[i][j],m1_raw_buf[i+col_size*j],UpperImage[i][j]); //test
		...
		for (k =0; k < M1_M2_RAWDATA_TEST_CNT; k++) {
			for (i = 0; i < ROW_SIZE; i++) {
				for (j = 0; j < col_size-1; j++) {
					TOUCH_I("%d / %d / %d\n",LowerImage[i][j],m2_raw_buf[k][i*col_size+j],UpperImage[i][j]);
			...
			if ((type == U3_JITTER_TEST) || (type == U3_BLU_JITTER_TEST) || (type == U0_JITTER_TEST))
			{
				...
				for (i = 0; i < ROW_SIZE; i++) {
					...
					TOUCH_I("[L] %d / %d / %d	[R] %d / %d / %d \n",
					0, m2_raw_average_buf[0][i], AverageImage[i][0], 0, m2_raw_average_buf[1][i], AverageImage[i][1]);
