/*
 * Code for handling MAUSB IN transfers.
 *
*/
...
struct mausbdev	*dev_in;
extern ktime_t timer_start;


extern void mausb_enqueue_ret_unlink(struct stub_device *sdev,
									 	 struct stub_mausb_pal *pal,
										 __u32 seqnum,
									     __u32 status);
extern void stub_free_mausb_pal_and_urb(struct stub_mausb_pal *pal);
extern int tweak_special_requests(struct urb *urb);
...
//		dev_info(&pal->urb->dev->dev, "No need to submit this request... skip!\n");
	...
	/* urb is now ready to submit */
        pal->urb->transfer_flags |= URB_NO_TRANSFER_DMA_MAP;
	...
	if (ret == 0) {
		...
		printk(KERN_INFO "IN: Time taken for urb submission: %u  milli seconds\n",(unsigned int)(actual_time));
#endif
	}	
	else {
...
int stub_pal_in_loop(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		wait_event_interruptible(sdev->pal_in_waitq,
					 (!list_empty(&sdev->mausb_pal_in_init) ||
					  kthread_should_stop()));
