...
/*
* Code for handling MAUSB Management transfers.
*
*/
...
void mausb_send_response(struct mausb_device *ud, struct mausb_header *header)
{
//	stub_send_mausb(ud,header);
}


void mausb_dev_handle_req(struct mausb_device *ud,
...
void mausb_dev_reset_req(struct mausb_device *ud,
									struct stub_mausb_pal *pal)
{
/*
	struct mausb_header *resp;
...
int mausb_mgmt_dissect_pkt(struct mausb_device *ud, struct stub_mausb_pal *pal)
{
    int offset = 0;
#if 0
	unsigned char *buff;
...
/*			kfree(pal->pdu);
			list_del(&pal->list);
			kmem_cache_free(stub_priv_cache, pal);
*/			
		}
}
