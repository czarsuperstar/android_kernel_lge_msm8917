...
//			LG_PRINT(DBG_LEVEL_MEDIUM,DATA_TRANS_RX,"Payload length: %lu\n",pkt_len);
			...
			printk(KERN_INFO "OUT: Time taken for data receive: %u  milli seconds\n",(unsigned int)(actual_time));
#endif			
		}
		else {
	...
	pal = kmem_cache_zalloc(stub_mausb_pal_cache, GFP_ATOMIC);
	if (pal == NULL )
	{
		LG_PRINT(DBG_LEVEL_MEDIUM,DATA_TRANS_RX,"kmem_cache_zalloc stub_mausb_pal_cache FAILED \n");
		if(packet)
		 kfree(packet);	
		return;
	}
...
//			timer_end = ktime_get();
//			actual_time = ktime_to_ms(ktime_sub(timer_end, timer_start));
//			LG_PRINT(DBG_LEVEL_MEDIUM,DATA_TRANS_RX,"OUT: Time taken for receiving: %u  milli seconds\n",(unsigned int)(actual_time));
