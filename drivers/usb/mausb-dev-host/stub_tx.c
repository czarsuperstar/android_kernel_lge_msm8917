...
void stub_free_mausb_pal_and_urb(struct stub_mausb_pal *pal)
{
	...
	if (pal)
	{
		...
		if (pal->pdu)
			kfree(pal->pdu);
		//LG_PRINT(DBG_LEVEL_MEDIUM,DATA_TRANS_TX,"IN 2\n");
		list_del(&pal->list);
		//LG_PRINT(DBG_LEVEL_MEDIUM,DATA_TRANS_TX,"IN 22222\n");
		kmem_cache_free(stub_mausb_pal_cache, pal);
		//LG_PRINT(DBG_LEVEL_MEDIUM,DATA_TRANS_TX,"IN 3\n");
		if (urb) {
			//LG_PRINT(DBG_LEVEL_MEDIUM,DATA_TRANS_TX,"IN 4\n");
			if (urb->transfer_buffer)
				kfree(urb->transfer_buffer);
			//LG_PRINT(DBG_LEVEL_MEDIUM,DATA_TRANS_TX,"IN 5\n");
			if (urb->setup_packet)
...
/**
 * stub_complete - completion handler of a mausb urb
...
//	LG_PRINT(DBG_LEVEL_MEDIUM,DATA_TRANS_TX,"---> %s\n",__func__);
	...
	mausb_dbg_stub_tx("complete! status %d\n", urb->status);
	...
	case 0:
		...
		break;
	case -ENOENT:
//		LG_PRINT(DBG_LEVEL_MEDIUM,DATA_TRANS_TX,"stopped by a call to usb_kill_urb() "
//			 "because of cleaning up a virtual connection\n");
		return;
	case -ECONNRESET:
//		LG_PRINT(DBG_LEVEL_MEDIUM,DATA_TRANS_TX, "unlinked by a call to "
//			 "usb_unlink_urb()\n");
		break;
	case -EPIPE:
//		LG_PRINT(DBG_LEVEL_MEDIUM,DATA_TRANS_TX, "endpoint %d is stalled\n",
//			 usb_pipeendpoint(urb->pipe));
		break;
	case -ESHUTDOWN:
//		LG_PRINT(DBG_LEVEL_MEDIUM,DATA_TRANS_TX, "device removed?\n");
		break;
	default:
//		LG_PRINT(DBG_LEVEL_MEDIUM,DATA_TRANS_TX,"urb completion with non-zero status "
//			 "%d\n", urb->status);
		break;
	}
...
//				ret, txsize);
		...
		printk( KERN_INFO "Time taken for transaction: %u  milli seconds\n",(unsigned int)(actual_time));	
#endif		
	return total_size;
}
