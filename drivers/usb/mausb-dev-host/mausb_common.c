...
#endif
static void mausb_iso_packet_correct_endian(
		struct mausb_iso_packet_descriptor *iso, int send)
{
...
static void mausb_pack_iso(struct mausb_iso_packet_descriptor *iso,
...
{
	struct mausb_iso_packet_descriptor *iso;
	...
	for (i = 0; i < np; i++) {
		mausb_pack_iso(&iso[i], &urb->iso_frame_desc[i], 1);
		mausb_iso_packet_correct_endian(&iso[i], 1);
	}
...
EXPORT_SYMBOL_GPL(mausb_alloc_iso_desc_pdu);
...
int mausb_recv_iso(struct mausb_device *ud, struct urb *urb)
{
	...
	ret = mausb_recv(ud->tcp_socket, buff, size);
	if (ret != size) {
	...
	iso = (struct mausb_iso_packet_descriptor *) buff;
	for (i = 0; i < np; i++) {
		mausb_iso_packet_correct_endian(&iso[i], 0);
		mausb_pack_iso(&iso[i], &urb->iso_frame_desc[i], 0);
...
EXPORT_SYMBOL_GPL(mausb_recv_iso);
...
EXPORT_SYMBOL_GPL(mausb_pad_iso);
